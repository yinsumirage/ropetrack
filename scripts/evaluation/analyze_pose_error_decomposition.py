...
from ropetrack.eval.dexycb import palm_orientation_error_deg, procrustes  # noqa: E402
...
def write_markdown(summary: dict, path: Path) -> None:
    ...
    lines += ["", "## C. DirectPose signed delta versus matched base", "", "Negative 螖E means improvement; headroom deltas describe changed oracle room and are not themselves accuracy improvements.", "", "| Dataset | Method | 螖Camera | 螖Root | 螖RT | 螖PA | 螖Translation headroom | 螖Rotation headroom |", "|---|---|---:|---:|---:|---:|---:|---:|"]
    for dataset, entry in summary["datasets"].items():
...
def make_judgment(config: dict, summary: dict, results: dict, groups_by_dataset: dict) -> tuple[dict, dict]:
    ...
    for spec in config["datasets"]:
        ...
        decisions[dataset] = {
            ...
            "direct_pose_effect": f"螖PA {local_delta:+.3f} mm; 螖camera {camera_delta:+.3f} mm",
    ...
    if len(qualifying) >= 3:
        if translation_support >= 3 and rotation_support >= 3:
            decision = "CONTINUE 螖R+螖t structured branch"
        elif rotation_support >= 3:
            decision = "CONTINUE 螖R only"
        else:
            decision = "CONTINUE 螖t only"
    else:
    ...
    return decisions, {
        ...
        "learnability_boundary": "This analysis proves oracle headroom and cross-dataset consistency only, not that 螖R or 螖t can be learned from the available inputs.",
    }
